...
export default function App() {
  ...
  useEffect(() => { 
      ...
      const manifestRef = doc(db, 'artifacts', appId, 'public', 'data', 'storage', 'manifest'); 
  ...
  const handleDeleteVisit = async (visitId) => { if (!user) return; try { await deleteDoc(doc(db, 'artifacts', appId, 'public', 'data', 'visits', visitId)); } catch (e) { console.error(e); } };
  const handleSaveVisit = async (newVisit) => { if (!user) return; try { await addDoc(collection(db, 'artifacts', appId, 'public', 'data', 'visits'), { ...newVisit, timestamp: new Date().toISOString() }); if (newVisit.contact) await addDoc(collection(db, 'artifacts', appId, 'public', 'data', 'client_updates'), { nClient: newVisit.nClient, contact: newVisit.contact }); alert("Visita guardada."); } catch (e) { console.error(e); } };
  const handleSaveQuote = async (newQuote) => { if (!user) return; try { await addDoc(collection(db, 'artifacts', appId, 'public', 'data', 'quotes'), { ...newQuote, timestamp: new Date().toISOString() }); alert("Cotización guardada."); } catch (e) { console.error(e); } };
